...
@SEGMENTORS.register_module()
class HRDAEncoderDecoder_clips(EncoderDecoder_clips):
    last_train_crop_box = {}
    ...
    def forward_train(self,
        ...
        if img.dim()==5:
            ...
            img = img.reshape(batch_size*num_clips, -1, h,w)
            # debug: the first 3 channels are image RGB, 4th is vanishing mask and 5th is global pos emb
            save_image(img[-1,:3,:,:], 'debug_img.png')
            save_image(img[-1,3:4,:,:], 'debug_vanishing_mask.png')
            save_image(img[-1,4:5,:,:], 'debug_pos_emb.png')
