...
class DAFormerSerialHead(BaseDecodeHead):
    ...
    def forward(self, inputs, feat_video, lr_out, hr_out):
        ...
        _c['feat_video'] = resize(feat_video[:,-1], size=os_size, mode='bilinear', align_corners=self.align_corners)
        _c['lr_out'] = resize(lr_out, size=os_size, mode='bilinear', align_corners=self.align_corners)
        _c['hr_out'] = resize(hr_out, size=os_size, mode='bilinear', align_corners=self.align_corners)
