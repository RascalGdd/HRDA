...
class TransHeadVideo(BaseDecodeHead):

    def __init__(self, **kwargs):
        super(TransHeadVideo, self).__init__(
        ...
        self.video_crs_attn = CrossAttentionLayer(
            d_model=hidden_dim,
            nhead=nheads,
            dropout=0.0,
            normalize_before=pre_norm,
    ...
    def forward(self, x, feat_video = None):
        # x is a list of multi-scale feature
        # torch.Size([1, 64, 128, 128])
        # torch.Size([1, 128, 64, 64])
        # torch.Size([1, 320, 32, 32])
        # torch.Size([1, 512, 16, 16])
        # feat_video: of size [1, 19, 128, 128]
        ...
        if feat_video is not None:
            outputs_mask = self.video_crs_attn(outputs_mask, feat_video)
